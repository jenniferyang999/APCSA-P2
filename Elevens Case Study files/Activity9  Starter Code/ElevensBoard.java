...
public class ElevensBoard extends Board {
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for (int k = 0; k < selectedCards.size(); k++)
		{
			...
			{
				if (cardAt(selectedCards.get(k)).pointValue() + cardAt(selectedCards.get(l)).pointValue() == 11)
				{
					return true;
				}
			}
		...
		for (int i = 0; i < selectedCards.size(); i++)
		{
			...
			{
				int l = selectedCards.get(j);

				if (cardAt(k).pointValue() == POINT_VALUES[j])
				{
					numb1 =  POINT_VALUES[j];
				}
				if (cardAt(l).pointValue() == POINT_VALUES[j])
				{
					numb2 = POINT_VALUES[j];
				}
			}
		}
		if (numb1 + numb2 == 11)
